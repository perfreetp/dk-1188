...
router.get('/', validate(schemas.search), async (req, res) => {
  try {
    ...
    if (type === 'all' || type === 'location') {
      const locations = await Location.searchByName(query);
      const filteredLocations = [];
      for (const loc of locations) {
        const travel = await Travel.findById(loc.travel_id);
        if (travel && travel.user_id === req.user.id) {
          filteredLocations.push({ id: loc.id, name: loc.name, type: loc.type, travelId: loc.travel_id, travelName: travel.name, checkInTime: loc.check_in_time });
        }
      }
    ...
    if (type === 'all' || type === 'companion') {
      const companions = await Companion.findByTravelId('');
      const filteredCompanions = [];
      for (const comp of companions || []) {
        if (comp.name.includes(query)) {
          const travel = await Travel.findById(comp.travel_id);
          if (travel && travel.user_id === req.user.id) {
            filteredCompanions.push({ id: comp.id, name: comp.name, role: comp.role, travelId: comp.travel_id, travelName: travel.name });
          }
        }
      }
    ...
    if (type === 'all' || type === 'content') {
      const snippets = await TextSnippet.search(query);
      const photos = await Photo.search(query);
      const filteredSnippets = [];
      for (const s of snippets) {
        const travel = await Travel.findById(s.travel_id);
        if (travel && travel.user_id === req.user.id) {
          filteredSnippets.push({ type: 'snippet', id: s.id, title: s.title, content: s.content?.substring(0, 100), travelId: s.travel_id, travelName: travel.name, date: s.date });
        }
      }
      ...
      for (const p of photos) {
        ...
        if (travel && travel.user_id === req.user.id) {
          filteredPhotos.push({ type: 'photo', id: p.id, url: p.url, description: p.description, travelId: p.travel_id, travelName: travel.name, takenAt: p.taken_at });
        }
      }
    ...
    res.json({ success: true, data: results, meta: { query, type, totalCount } });
  } catch (error) {
